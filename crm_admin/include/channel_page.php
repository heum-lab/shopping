<?php
/**
 * 6개 보조 채널(플레이스/블로그/유입플/오늘의집/카카오맵/자동완성) 공통 렌더러.
 *
 * 각 채널 페이지는 아래 변수를 정의한 뒤 이 파일을 include 한다.
 *   $channel_config = [
 *       'key'      => 'place',                              // 식별자
 *       'label'    => '플레이스',                            // UI 타이틀
 *       'table'    => 'place_work',                         // DB 테이블 (화이트리스트 검증)
 *       'page_url' => '/crm_admin/sub/info_acc_place.php',  // 본인 URL
 *   ];
 *
 * db.php / session.php / func.php / header/footer 는 각 래퍼 페이지에서 require_once 한다.
 */

// ------------------------------------------------------------------
// 0. 화이트리스트 검증 (SQL injection 방지)
// ------------------------------------------------------------------
$ALLOWED_CHANNEL_TABLES = [
    'place_work', 'blog_work', 'inflow_work',
    'ohouse_work', 'kakao_work', 'auto_work',
];
if (empty($channel_config) || !in_array($channel_config['table'] ?? '', $ALLOWED_CHANNEL_TABLES, true)) {
    http_response_code(500);
    die('Invalid channel configuration.');
}

$ch_key   = $channel_config['key'];
$ch_label = $channel_config['label'];
$table    = $channel_config['table'];
$page_url = $channel_config['page_url'];

if (!defined('PAGE_TITLE')) define('PAGE_TITLE', $ch_label . ' — CONTROL');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $mode !== '') {
    ...
    $return_url = p('return_url', $page_url);
    ...
    if ($mode === 'insert') {
        ...
        mysqli_query($conn, "INSERT INTO {$table} ({$cols}) VALUES ({$vals})");
        $new_idx = (int)mysqli_insert_id($conn);
        audit_log('insert', $table, $new_idx, [
            'channel' => $ch_key, 'keyword' => $fields['keyword'],
    ...
    $check_idx = array_filter(array_map('intval', (array)$check_idx), fn($v) => $v > 0);

    if (!empty($check_idx)) {
        $idx_in = implode(',', $check_idx);

        audit_log('bulk_update', $table, null, [
            'channel' => $ch_key, 'mode' => $mode, 'ids' => $check_idx,
            'count' => count($check_idx), 'extend_days' => (int)p('extend_days', 0),
        ]);

        // 대행사(level=3) 권한자는 본인이 등록한 작업만 대상으로 제한
        $scope_sql = ($admin_level === 3) ? " AND seller_idx = " . (int)$admin_idx : '';
                ...
                if ($days > 0) {
                    mysqli_query($conn,
                        "UPDATE {$table}
                         ...
                         WHERE idx IN ({$idx_in}){$scope_sql}"
                    );
                }
                break;
            case 'move_working':
                mysqli_query($conn, "UPDATE {$table} SET status = '작업중' WHERE idx IN ({$idx_in}){$scope_sql}");
                break;
            case 'refund_req':
                mysqli_query($conn, "UPDATE {$table} SET status = '환불요청', refund_date = CURDATE() WHERE idx IN ({$idx_in}){$scope_sql}");
                break;
            case 'delete_req':
                mysqli_query($conn, "UPDATE {$table} SET status = '삭제요청' WHERE idx IN ({$idx_in}){$scope_sql}");
                break;
            case 'del_mode':
                if (!is_super()) { require_super(); }
                mysqli_query($conn, "DELETE FROM {$table} WHERE idx IN ({$idx_in}){$scope_sql}");
                break;
        }
    }
    header('Location: ' . $return_url);
...
$cnt_row   = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT COUNT(*) AS cnt FROM {$table} w
...
<div class="d-flex justify-content-between align-items-center mb-2">
    <h2 class="page-title mb-0"><?= h($ch_label) ?> 작업 관리 <span class="text-muted small">총 <?= number_format($total) ?>건</span></h2>
    <div>
        <?php if ($admin_level <= 3): ?>
            <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#showModalInsert">+ 작업 등록</button>
            <a class="btn btn-sm btn-outline-secondary" href="/crm_admin/ajax/xls.download.ajax.php?channel=<?= h($ch_key) ?>&amp;xls_mode=template">엑셀 양식</a>
        <?php endif; ?>
        <a class="btn btn-sm btn-outline-secondary" href="/crm_admin/ajax/xls.download.ajax.php?channel=<?= h($ch_key) ?>&amp;<?= h(build_qs(['xls_mode' => 'search'])) ?>">검색결과 다운로드</a>
        <?php if ($admin_level <= 3): ?>
...
<form name="searchfrm" method="get" class="search-box">
            ...
            <div class="col-md-2">
                ...
                <select name="pass_assign3" class="form-select form-select-sm">
                    ...
                    <?php mysqli_data_seek($agency_res, 0); while ($a = mysqli_fetch_assoc($agency_res)): ?>
                        <option value="<?= (int)$a['idx'] ?>" <?= (string)$pass_assign3 === (string)$a['idx'] ? 'selected' : '' ?>><?= h($a['name']) ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
        <?php endif; ?>
            ...
            <div class="col-md-2">
                ...
                <select name="pass_assign" class="form-select form-select-sm">
                    ...
                    <?php mysqli_data_seek($seller_res, 0); while ($s = mysqli_fetch_assoc($seller_res)): ?>
                        <option value="<?= (int)$s['idx'] ?>" <?= (string)$pass_assign === (string)$s['idx'] ? 'selected' : '' ?>><?= h($s['name']) ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
        <?php endif; ?>
    ...
    <div class="row g-2 align-items-end mt-1">
        ...
        <div class="col-md-2 d-flex gap-1">
            ...
            <a href="<?= h($page_url) ?>" class="btn btn-sm btn-outline-secondary">초기화</a>
        </div>
    </div>
</form>

<!-- 일괄 처리 폼 -->
<form name="check_form2" method="post" action="<?= h($page_url) ?>">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover data-table align-middle">
            ...
            <tbody>
            <?php $col_count = $admin_level <= 3 ? 11 : 10; ?>
            <?php if ($total === 0): ?>
            ...
            ?>
                <tr>
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-sm btn-outline-primary py-0 px-1" onclick="channelEdit('<?= h($ch_key) ?>', <?= (int)$r['idx'] ?>, '<?= h($_SERVER['REQUEST_URI']) ?>');">상세</button>
                    </td>
                </tr>
            <?php endwhile; endif; ?>
            </tbody>
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?= h($ch_label) ?> 작업 상세</h5>
...
<div class="modal fade" id="showModalInsert" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" action="<?= h($page_url) ?>">
                <input type="hidden" name="mode" value="insert">
                <input type="hidden" name="return_url" value="<?= h($_SERVER['REQUEST_URI']) ?>">
                <div class="modal-header">
                    <h5 class="modal-title"><?= h($ch_label) ?> 작업 등록</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-2">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="keyword_type" class="form-control form-control-sm" placeholder="<?= h($ch_label) ?>별 타입 자유 입력">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="product_mid" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="inflow_count" class="form-control form-control-sm" value="0" min="0">
                        </div>
                        <div class="col-md-4">
...
<div class="modal fade" id="showModalUpload" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?= h($ch_label) ?> 엑셀 대량 등록</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-light small">
                    <strong>안내:</strong>
                    <ol class="mb-0 ps-3">
                        <li><a href="/crm_admin/ajax/xls.download.ajax.php?channel=<?= h($ch_key) ?>&amp;xls_mode=template">엑셀 양식 다운로드</a> 후 작성하세요.</li>
                ...
                <form id="uploadForm" enctype="multipart/form-data">
                    <input type="hidden" name="channel" value="<?= h($ch_key) ?>">
                    <div class="mb-2">
...
<script>
function channelEdit(channel, idx, return_url) {
    $.post('/crm_admin/ajax/channel.info.ajax.php',
        { channel: channel, idx: idx, return_url: return_url },
        function (html) {
